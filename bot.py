...
from aiogram.types import (
    ...
    ReplyKeyboardRemove, MessageEntity, BufferedInputFile
)
...
async def resolve_photo(photo: str) -> str | BufferedInputFile:
    """Telegram часто не может скачать крупные PNG по URL с GitHub — отдаём файлом."""
    if not photo.startswith(("http://", "https://")):
        return photo
    if photo not in _photo_bytes_cache:
        dl_timeout = ClientTimeout(total=120, connect=30, sock_read=120)
        async with aiohttp.ClientSession(timeout=dl_timeout) as session:
            async with session.get(photo) as resp:
                resp.raise_for_status()
                data = await resp.read()
        if not data.startswith((b"\x89PNG\r\n\x1a\n", b"\xff\xd8\xff")):
            raise ValueError(f"URL не содержит изображение: {photo}")
        _photo_bytes_cache[photo] = data
    filename = photo.rsplit("/", 1)[-1].split("?")[0] or "photo.png"
    return BufferedInputFile(_photo_bytes_cache[photo], filename=filename)

async def send_photo(chat_id: int, photo: str, **kwargs):
    resolved = await resolve_photo(photo)
    kwargs.setdefault("request_timeout", 300)
    last_err: Exception | None = None
    for attempt in range(3):
        try:
            await bot.send_photo(chat_id, resolved, **kwargs)
            return
        except (TelegramNetworkError, asyncio.TimeoutError) as e:
            last_err = e
            wait = 5 * (attempt + 1)
            logger.warning(f"send_photo retry {attempt + 1}/3 in {wait}s: {e}")
            await asyncio.sleep(wait)
    raise last_err  # type: ignore[misc]

async def warmup_photos():
    for url in (PHOTO_8_UZHAS, PHOTO_8_UGAR, PHOTO_10, PHOTO_11, PHOTO_13):
        await resolve_photo(url)
        logger.info(f"photo cached: {url.rsplit('/', 1)[-1]}")
...
async def continue_after_scenario(chat_id: int, chosen: str):
    if chat_id in _active_scenarios:
        logger.warning(f"[{chat_id}] scenario already running, skip")
        return
    _active_scenarios.add(chat_id)
    try:
        ...
        await send_photo(chat_id, photo_id, caption=MSG8_CAPTION, caption_entities=E(MSG8_ENT))
        ...
        await send_photo(chat_id, PHOTO_10, caption=MSG10_CAPTION, caption_entities=E(MSG10_ENT))

        await asyncio.sleep(5)
        logger.info(f"[{chat_id}] sending msg11")
        await send_photo(chat_id, PHOTO_11, caption=MSG11_CAPTION, caption_entities=E(MSG11_ENT))
        ...
        await send_photo(chat_id, PHOTO_13, caption=MSG13_CAPTION, caption_entities=E(MSG13_ENT), reply_markup=kb_book())
        ...
        _active_scenarios.discard(chat_id)
...
async def main():
    ...
    try:
        ...
        logger.error(f"photo warmup failed: {e}")
