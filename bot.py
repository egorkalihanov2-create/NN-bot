...
async def resolve_photo(photo: str) -> str | BufferedInputFile:
    ...
    if photo not in _photo_bytes_cache:
        async with bot.session.get(photo) as resp:
...
async def send_photo(chat_id: int, photo: str, **kwargs):
    await bot.send_photo(chat_id, await resolve_photo(photo), **kwargs)
...
async def main():
    # Снимаем webhook и старые апдейты — иначе Conflict при втором инстансе polling
